 ...
 * 使用bean_xml.xml配置事务（tx:advice + aop:config）
 * 1. 充值500点券 -> 网络延迟超过5秒 -> 事务回滚
 * 2. 消费300点券 -> 正常提交 -> 记录到records表，更新acc的coupon
 */
public class TestXml {
    public static void main(String[] args) {
        ApplicationContext ctx = new ClassPathXmlApplicationContext("bean_xml.xml");

        CouponService service = (CouponService) ctx.getBean("couponServiceXml");
